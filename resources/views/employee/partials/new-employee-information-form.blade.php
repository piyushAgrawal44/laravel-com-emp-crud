<section>
    ...
    <form method="post" action="{{ route('employee.create') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <select id="company_id" name="company_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'new-employee')
                <p
                    x-data="{ show: true }"
                    x-show="show"
                    x-transition
